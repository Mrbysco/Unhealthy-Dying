    ...
    private static int addHearts(CommandContext<CommandSource> ctx, boolean silent) throws CommandSyntaxException {
        ...
        for(ServerPlayerEntity player : EntityArgument.getPlayers(ctx, "player")) {
            if(health > 0) {
                ...
                if(!silent) {
                    ITextComponent text = new TranslationTextComponent("unhealthydying:addhearts.message", (double)health/2).applyTextStyle(TextFormatting.RED);
    ...
    private static int removeHearts(CommandContext<CommandSource> ctx, boolean silent) throws CommandSyntaxException {
        ...
        for(ServerPlayerEntity player : EntityArgument.getPlayers(ctx, "player")) {
            if(health > 0) {
                ...
                if(!silent) {
                    ITextComponent text = new TranslationTextComponent("unhealthydying:removehearts.message", (double)health/2).applyTextStyle(TextFormatting.RED);
                    ctx.getSource().sendFeedback(text, false);
                }
            }
        }
    ...
    private static int setHearts(CommandContext<CommandSource> ctx, boolean silent) throws CommandSyntaxException {
        ...
        for(ServerPlayerEntity player : EntityArgument.getPlayers(ctx, "player")) {
            if(health > 0) {
                ...
                if(!silent) {
                    ITextComponent text = new TranslationTextComponent("unhealthydying:sethealth.message", health).applyTextStyle(TextFormatting.RED);
                    ctx.getSource().sendFeedback(text, false);
                }
            }
        }
