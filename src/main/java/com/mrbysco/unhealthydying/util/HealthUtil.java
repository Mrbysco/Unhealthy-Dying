...
public class HealthUtil {
	public static void sendHealthMessage(Player player, int newHealth, int gained) {
		if (gained >= 0 && UnhealthyConfig.SERVER.regenHealthMessage.get()) {
			MutableComponent text = Component.translatable("unhealthydying:regennedHealth.message", newHealth + gained).withStyle(ChatFormatting.DARK_GREEN);
			player.displayClientMessage(text, true);
		} else {
			if (UnhealthyConfig.SERVER.reducedHealthMessage.get()) {
				MutableComponent text = Component.translatable("unhealthydying:reducedHealth.message", newHealth + gained).withStyle(ChatFormatting.DARK_RED);
				player.displayClientMessage(text, true);
			}
		}
	}
